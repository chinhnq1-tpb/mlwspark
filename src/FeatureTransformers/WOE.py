...
class ProcessBinningModel(
    Transformer, HasInputCols, DefaultParamsReadable, DefaultParamsWritable
):
    pickledBin = Param(
        Params._dummy(),
        "pickledBin",
        "The pickled BinningProcess object, base64 encoded",
    )

    @keyword_only
    def __init__(self, inputCols=None, pickledBin=None) -> None:
        super().__init__()
    ...
    @keyword_only
    def set_params(self, inputCols=None, pickledBin=None) -> None:
        kwargs = self._input_kwargs
        self._set(**kwargs)
...
class ProcessBinningEstimator(
    Estimator, HasInputCols, HasLabelCol, DefaultParamsReadable, DefaultParamsWritable
):
    woeCols = Param(
        Params._dummy(),
        "woeCols",
    ...
    @keyword_only
    def __init__(self, inputCols: list, labelCol: str, woeCols: list = []) -> None:
        super().__init__()
    ...
    @keyword_only
    def set_params(self, inputCols: list, labelCol: str, woeCols: list = []) -> None:
        kwargs = self._input_kwargs
        self._set(**kwargs)
        if woeCols == []:
